...
const Category = styled.div<{ selected: boolean, key?: number }>`
    position: relative;
    border: 1px solid ${props => props.selected ? 'var(--yellow)' : 'transparent'};
    cursor: pointer;
    border-radius: 30px;
    padding: 25px 50px;
    font-size: 14px;
...
const CategorySlider: FC<Props> = () => {
    ...
    const seeAll = () => {
        if (selected === 0) {
            ...
            history.push(`/productos/${toLink(data.categories.find((row: CategoryType) => row.entity_id === selected).name)}`);
        }
    }
    ...
    if (selected > 0) {
        categoryName = data.categories.find((row: CategoryType) => row.entity_id === selected).name;
    }

    return <Suspense fallback={<Loader />}>
        <div className="main-container">
            <CategoryList>
                <Category onClick={() => setSelected(0)} selected={selected === 0}>{t('homepage.categoryslider.all')}</Category>
                {!loading && data && data.categories.map((row: CategoryType) =>
                    <Category onClick={() => setSelected(row.entity_id)} selected={selected === row.entity_id} key={row.entity_id}>
                        {row.name}
                    </Category>)
                }
            </CategoryList>
            <CategoryMobile>
                ...
                {open && <CategoryMobileList>
                    <CategoryItemMobile onClick={() => selectCategory(0)} selected={selected === 0}>{t('homepage.categoryslider.all')}</CategoryItemMobile>
                    {!loading && data && data.categories.map((row: CategoryType) =>
                        <CategoryItemMobile onClick={() => selectCategory(row.entity_id)} selected={selected === row.entity_id} key={row.entity_id}>
                            {row.name}
                        </CategoryItemMobile>)
                    }
                </CategoryMobileList>}
